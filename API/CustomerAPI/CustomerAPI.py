...
API_ADDRESSBOOK = {"FT" : FT_URL,"LT" : LT_URL,  "ST" : ST_URL}
...
def GetAllEntities():
    q = "use customerdata MATCH (n) RETURN n"
...
def GetOutwardNodes(Nodes):
    q = "use customerdata MATCH (n) WHERE n.name IN " + str(Nodes) + " MATCH (n)-[:EMPLOYED_BY*1..]->(m) RETURN m"
